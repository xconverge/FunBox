...
void ImpulseResponse::setImpulseResponse(const float* ir, uint32_t len,
                                         bool norm) {
  if (len > DEFAULT_FIR_LEN) len = DEFAULT_FIR_LEN;
  for (uint32_t i = 0; i < len; ++i) fir_coeffs_[i] = ir[i];
  if (norm) normalise(fir_coeffs_, len);
...
void ImpulseResponse::normalise(float* c, uint32_t len) {
  float32_t sum = 0.0f;
  for (uint32_t i = 0; i < len; ++i) sum += fabsf(c[i]);
  if (sum > 0.0f) {
    const float32_t k = 1.0f / sum;
